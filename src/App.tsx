  ...
  render() {
    ...
    if (hasError) {
      ...
      try {
        ...
        if (parsed.error && parsed.operationType) message = `Firestore ${parsed.operationType} error: ${parsed.error}`;
      } catch {
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        const userDoc = await getDoc(doc(db, 'users', firebaseUser.uid)).catch((e) =>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => items.push({ ...d.data(), id: d.id } as LibraryItem));
        setLibrary(items);
      },
    ...
    try {
      if (isSignup) {
        ...
        const newUser: AppUser = {
          ...
          firstName,
          lastName,
          role: cred.user.email === 'worshipwarehousesite@gmail.com' ? 'admin' : 'user',
          subscriptionStatus: 'inactive',
          createdAt: new Date().toISOString(),
        };
        await setDoc(doc(db, 'users', cred.user.uid), newUser).catch((e) =>
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setAuthError('Email/Password sign-in is not enabled. Enable it in Firebase Console.');
      } else {
  ...
  const handleSubscribe = async (priceId: string) => {
    ...
    if (!stripeConfig?.stripeConfigured) {
      setAuthError('Stripe is not configured.');
      return;
    }
    if (!priceId) {
      setAuthError('Invalid price ID.');
      return;
    }
    try {
      setAuthError('');
  ...
  const onLoadFromLibrary = (item: LibraryItem) => {
    if (item.type === 'song') {
      ...
      const data = {
        title: item.title,
        key: item.key || '',
        ccliInfo: item.ccliInfo || '',
        primaryText: item.primaryText,
        secondaryText: item.secondaryText,
        linesPerLanguage: item.linesPerLanguage,
      };
      if (songMode === 'stage-chord') {
        setStageChordInitial({ ...data });
        setActivePage('stage-chord');
      } else {
        setMultilingualInitial({ ...data });
        setActivePage('multilingual');
      }
    } else {
            ...
            <>
                  ...
                  {navItems.map(({ page, icon: Icon, label }) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                        activePage === page ? 'bg-white shadow-sm text-neutral-900' : 'text-neutral-500 hover:text-neutral-900'
                      }`}
                    >
                      <Icon size={13} />
                      {label}
              ...
              <div className="flex items-center gap-2 pl-3 border-l border-neutral-200">
                <div className="flex items-center gap-2 px-3 py-1 bg-neutral-100 rounded-full">
                  <div className="w-5 h-5 bg-neutral-900 rounded-full flex items-center justify-center text-white">
                    <User size={11} />
                  </div>
                  <span className="text-xs font-medium max-w-[120px] truncate hidden sm:block">{user?.email}</span>
                </div>
                <button onClick={handleLogout} className="p-1.5 text-neutral-400 hover:text-red-600 transition-colors" title="Logout">
                  <LogOut size={16} />
                </button>
              </div>
            </>
          ...
          {navItems.map(({ page, icon: Icon, label }) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold whitespace-nowrap transition-all ${
                activePage === page ? 'bg-neutral-900 text-white' : 'text-neutral-500 hover:bg-neutral-100'
              }`}
            >
              <Icon size={13} />
              {label}
        ...
        {isAuthLoading ? (
            ...
            <div className="space-y-16 py-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                {[
                  ...
                  <div key={i} className="bg-white p-8 rounded-2xl border border-neutral-200 space-y-4">
                    <div className={`w-11 h-11 ${f.color} rounded-xl flex items-center justify-center`}>
                      <f.icon size={20} />
                    </div>
              ...
              <section className="bg-neutral-900 text-white p-12 rounded-3xl text-center space-y-6 max-w-4xl mx-auto">
                ...
                <p className="text-neutral-400 max-w-xl mx-auto">
                  Join churches worldwide using WorshipSlides Pro.
                </p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto mt-16">
              <div className="bg-white p-8 rounded-2xl border border-neutral-200 shadow-lg space-y-6">
                <div className="text-center space-y-2">
                  ...
                  <div className="w-14 h-14 bg-neutral-900 rounded-xl flex items-center justify-center text-white mx-auto mb-4">
                    <Lock size={28} />
                  </div>
                ...
                <form onSubmit={handleLogin} className="space-y-3">
                  ...
                  <div className="relative py-3">
                    ...
                    <div className="relative flex justify-center text-[10px] uppercase tracking-widest font-bold"><span className="bg-white px-3 text-neutral-400">or</span></div>
                  </div>

                  <button type="button" onClick={handleGoogleLogin} className="w-full py-3 bg-white border border-neutral-200 text-neutral-900 rounded-xl font-bold hover:bg-neutral-50 transition-all flex items-center justify-center gap-3 shadow-sm">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-xl mx-auto mt-8 space-y-6">
            <div className="bg-white p-10 rounded-2xl border border-neutral-200 shadow-lg text-center space-y-6">
              ...
              <button onClick={handleLogout} className="text-sm text-neutral-400 hover:text-red-600 font-medium flex items-center gap-2 mx-auto pt-2">
          ...
          <AnimatePresence mode="wait">
            {activePage === 'dashboard' && (
              <DashboardPage setActivePage={setActivePage} library={library} onLoadFromLibrary={onLoadFromLibrary} />
            )}
            {activePage === 'sermon' && (
              <SermonPage user={user} setActivePage={setActivePage} />
            )}
            {activePage === 'multilingual' && (
              <MultilingualPage user={user} setActivePage={setActivePage} initialData={multilingualInitial} />
            )}
            {activePage === 'stage-chord' && (
              <StageChordPage user={user} setActivePage={setActivePage} initialData={stageChordInitial} />
            )}
            {activePage === 'library' && (
              <LibraryPage library={library} onLoadFromLibrary={onLoadFromLibrary} onRemoveFromLibrary={onRemoveFromLibrary} setActivePage={setActivePage} />
            )}
            {activePage === 'profile' && (
              <ProfilePage user={user} setUser={setUser} isSubscribed={isSubscribed} stripeConfig={stripeConfig} onSubscribe={handleSubscribe} authError={authError} setActivePage={setActivePage} />
            )}
          </AnimatePresence>
        )}
      </main>
