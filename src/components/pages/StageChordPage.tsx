...
import { Music, Plus, ChevronDown, ChevronUp, ArrowLeft, Settings, Info, Upload } from 'lucide-react';
...
export const StageChordPage: React.FC<StageChordPageProps> = ({ user, setActivePage, initialData }) => {
  ...
  const handleChordProUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => setPrimaryText(ev.target?.result as string);
  ...
  const songData = useMemo((): SongData => {
    ...
    while (pIdx < primaryLinesRaw.length) {
      ...
      if (pBatch.length > 0) {
        slides.push({
          ...
          chords: [],
          notes: stageNotesBatch.join('\n'),
        });
      }
    }
    return { title, artist: '', key, slides };
  }, [title, key, primaryText, linesPerLanguage]);
  ...
  const exportPPTX = () => {
    ...
    songData.slides.forEach(slide => {
      ...
      pSlide.addText(slide.primaryLines.join('\n'), {
        x: '5%', y: '15%', w: '90%', h: '70%',
        align: 'center', valign: 'middle', color: 'FFFFFF', fontSize: 44, fontFace: 'Arial', bold: true,
      });
  ...
  return (
    <motion.div key="stage-chord" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-neutral-900">Stage Chord Mode</h2>
          <p className="text-neutral-500 text-sm">Single-language slides with chords embedded in ProPresenter slide notes.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-5">
          <div className="bg-white p-5 rounded-2xl border border-neutral-200 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={key} onChange={e => setKey(e.target.value)} className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900/5" placeholder="e.g. G" />
            </div>
          </div>
          ...
          <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 space-y-2">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-emerald-700 flex items-center gap-2"><Music size={12} /> How It Works</h3>
            <p className="text-sm text-emerald-700 leading-relaxed">
              Upload a ChordPro file or type lyrics with chord brackets. Audience sees clean lyrics; musicians see chords and lyrics in ProPresenter slide notes.
            </p>
          </div>
        ...
        <div className="lg:col-span-8 space-y-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-neutral-400 flex items-center gap-2"><Music size={14} /> Lyrics</h3>
              ...
              <span className="text-[10px] font-bold bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded">CHORD PRO SUPPORTED</span>
            </div>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {songData.slides.map((slide, idx) => (
                  <SongSlidePreview key={slide.id} slide={slide} index={idx} mode="stage-chord" showChords={true} />
                ))}
              </div>
            </motion.div>
          )}
